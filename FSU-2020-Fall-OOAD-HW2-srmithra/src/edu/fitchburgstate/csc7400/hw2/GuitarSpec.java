/**
 * Class: Object-Oriented Design and Analysis
 * Professor: Orlando Montalvo
 * Assignment: HW 2
 * Author - Sai Sree Mithra Sripathi
 * 
 * Date: 2018-10-01
 */
package edu.fitchburgstate.csc7400.hw2;
/**
 * GuitarSpec contains the information needed to keep track of a type of guitar specifications from
 * Rick's music store
 * 
 * @author HeadFirstOODA- Sai Sree Mithra Sripathi
 */

public class GuitarSpec implements GuitarSpecInterface {
	private GuitarManufacturer gmf; 
	private String model;
	private GuitarType type;
	private GuitarWood backGuitarWood;
	private GuitarWood topGuitarWood;
	/**
	 * Full constructor for GuitarSpec
	 * @param gmf
	 * @param model
	 * @param type
	 * @param backGuitarWood
	 * @param topGuitarWood
	 */
	public GuitarSpec(GuitarManufacturer gmf, String model, GuitarType type,
            GuitarWood backGuitarWood, GuitarWood topGuitarWood) {
		this.gmf = gmf;
		this.model = model;
		this.type = type;
		this.backGuitarWood = backGuitarWood;
		this.topGuitarWood = topGuitarWood;
	}
	/**
	 * returns the Guitar Manufacturer from the GuitarManufacturer enum
	 */
	@Override
	public GuitarManufacturer getGuitarManufacturer() {
		return gmf;
	}
	/**
	 * 
	 * returns a string, which defines the Model of the Guitar
	 */
	@Override
	public String getModel() {
		return model;
	}
	/**
	 * Returns type of guitar from GuitarType enum
	 * @return the type
	 */
	@Override
	public GuitarType getGuitarType() {
		return type;
	}
	/**
	 * Returns backWood of Guitar which belongs to GuitarWood enum
	 * @return the backGuitarWood
	 */
	@Override
	public GuitarWood getBackGuitarWood() {
		return backGuitarWood;
	}
	/**
	 * Returns topWood of Guitar which belongs to GuitarWood enum 
	 * @return the topGuitarWood
	 */
	@Override
	public GuitarWood getTopGuitarWood() {
		return topGuitarWood;
	}
	/**
	 * checks the guitar specs - manufacturer , model , type , back wood and top wood 
	 * if they are null / empty or whether they match the existing guitar entries
	 * null or empty are not accepted
	 * if the values passed to the method as parameter and the existing values do not match it is not accepted. 
	 * in case any condition is not met it returns false value. 
	 * if all conditions it finally returns a true value.
	 * @param Guitar spec containing details about manufacturer , model , type , back wood and top wood
	 * @returns boolean value - True if guitar passed as argument matches existing guitar
	 * False if any of the spec values is null or empty or doesnt match with the existing guitar values.
	 */
	@Override
	public boolean matches(GuitarSpec otherSpec) {
		if ((gmf != null) && (!gmf.equals("")))
	    if (gmf != otherSpec.gmf)
	      return false;
	    if ((model != null) && (!model.equals("")))
	        if((!model.equals(otherSpec.model)))
	      return false;
	    if ((type != null) && (!type.equals("")))
	    	if(type != otherSpec.type)
	      return false;
	    if ((backGuitarWood != null) && (!backGuitarWood.equals("")) )
	    	if(backGuitarWood != otherSpec.backGuitarWood)
	      return false;
	    if ((topGuitarWood != null) && (!topGuitarWood.equals(""))) 
	    	if(topGuitarWood != otherSpec.topGuitarWood)
	      return false;
	    return true;
	  }
}
